...
object HandRank {
  ...
  def isStraight(cards : List[Card]) : (Boolean, Int) = {
    def _isStraight(cards : List[Card], count : Int, kicker : Int) : (Int,Int)= {
      cards match {
        case List(c, c2 : List[Card]) => {
          if(c2.head.Value() == c.Value() - 1) _isStraight(cards.tail, count + 1, if(count == 0) c.Value() else kicker)
          else if(c2.head.Value() == c.Value()) _isStraight(cards.tail, count, if(count == 0) c.Value() else kicker)
          else _isStraight(cards.tail, 0, -1)
        }
        case _ => (count, kicker)
      }
    }
